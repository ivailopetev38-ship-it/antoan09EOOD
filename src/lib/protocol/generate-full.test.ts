...
const data: ProtocolData = {
  ...
  handedBy: 'Г. Предавателев', receivedBy: 'П. Приемателев',
  lines,
};
...
describe('Рендер в Word (.docx) — всяка графа присъства в документа', () => {
  ...
  it('подписи ПРЕДАЛ/ПРИЕЛ — попълваеми (+ представител в увода)', () => {
    expect(text).toContain('Г. Предавателев'); // ПРЕДАЛ + представителят в увода
    expect(text).toContain('П. Приемателев');  // ПРИЕЛ
  });
  it('подписи по подразбиране (undefined) — В. Вълков (предал) + собственикът (приел)', () => {
    const t2 = plainText(generateProtocolDocx({ ...data, handedBy: undefined, receivedBy: undefined }));
    expect(t2).toContain('В. Вълков');
    // приел по подразбиране = ownerName → „Тест Клиент ЕООД" се среща и в „Собственик…", и в ПРИЕЛ
    expect((t2.match(/Тест Клиент ЕООД/g) || []).length).toBeGreaterThanOrEqual(2);
  });
  it('изрично празно „Предал" → НЕ слага скрито „В. Вълков"', () => {
    const t2 = plainText(generateProtocolDocx({ ...data, handedBy: '', receivedBy: '' }));
    expect(t2).not.toContain('В. Вълков');
  });
